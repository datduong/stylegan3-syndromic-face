...
from pytorch_ssim import SSIM  # from https://github.com/Po-Hsun-Su/pytorch-ssim # ! outdated, so use this https://pytorch.org/ignite/generated/ignite.metrics.SSIM.html? 
...
def ParseClassLabel(string, n=18): 
    # @n len of 1-hot index
    index = [0] * n 
...
def project(
    ...
    if C is not None: 
        C = torch.LongTensor ( np.repeat ( np.expand_dims(C,axis=0), w_avg_samples, axis=0) ) # match shape of z_samples (10000, 512) 
        C = C.to(device)
        
    # Compute w stats.
    z_samples = np.random.RandomState(123).randn(w_avg_samples, G.z_dim) # shape z_samples (10000, 512) 
    w_samples = G.mapping(torch.from_numpy(z_samples).to(device), C)  # [N, L, C] # ! add class label, otherwise C=None
    if project_in_wplus:  # Thanks to @pbaylies for a clean way on how to do this
        ...
        if start_wavg:
            ...
            w_avg = G.mapping(torch.from_numpy(z).to(device), C)  # [1, L, C]
    ...
    elif loss_paper == 'clip': # @https://github.com/openai/CLIP
        import clip
...
@click.option('--class-str', '-class_str', type=str, help='Class label index in 1-hot array b x 1 x C', default=None)
@click.option('--class-len', '-class_len', type=int, help='length of 1-hot label', default=None)
@click.option('--lump-outdir', 'lump_outdir', is_flag=True, help='Put all projections into same outdir')
...
def run_projection(
    ...
    # ! parse label
    C = ParseClassLabel(class_str,class_len) if class_str is not None else None
    ...
    run_dir = gen_utils.make_run_dir(outdir, desc, lump_outdir=lump_outdir)
    ...
    target_name = re.sub( r'\.png','',target_fname.split('/')[-1] ) # traceable name
    result_name = os.path.join(run_dir, 'proj' + target_name) 
    npy_name = os.path.join(run_dir, 'projected' + target_name)
    ...
    target_pil.save(os.path.join(run_dir, target_name + '.jpg')) 
