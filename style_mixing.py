...
# ! added 
@click.option('--W_vec_dir', 'W_vec_dir', type=str, help='Directory path to already projected images', default=None, show_default=True)
@click.option('--save_each_img', 'save_each_img', is_flag=True, help='Directory path to already projected images')
...
def generate_style_mix(
    ...
    if W_vec_dir is None: 
        # ! generate W if not provided 
        ...
        # ! load in W 
        noise_mode = 'const' # ! because projection runs with 'const' setting. style-mix should be 'const' too
        w_dict = {}
        all_images = []
        npy_list = sorted ( [ npw for npw in os.listdir(W_vec_dir) if npw.endswith('npy') ] )
        print ('npy_list', npy_list)
        for seed, f in zip(all_seeds, npy_list ) : 
            w_dict [ seed ] = torch.from_numpy ( np.load( os.path.join(W_vec_dir,f) ) ).squeeze(0).to(device) # ! append seed with image into dict, @npy is saved as 3D, so we squeeze to get 2D. see @w_to_img
            all_images.append ( gen_utils.w_to_img(G, w_dict [ seed ], noise_mode)[0] ) # return 1x256x256x3 so take out first entry
        #
        image_dict = {(seed, seed): image for seed, image in zip(all_seeds, list(all_images))}
    ...
    for row_seed in row_seeds:
        for col_seed in col_seeds:
            ...
            w[col_styles] = w_dict[col_seed][col_styles] # ! @w should be 2D for @w[col_styles]
            # @w_to_img will unsqueeze to conver 2D @w into 3D. 
    ...
    W = G.img_resolution
    H = G.img_resolution
    print('G.synthesis.img_channels', G.synthesis.img_channels)
