...
@click.option('--direction-vec', '-direction_vec', type=str, help='direction_vec', default=None, show_default=True)
@click.option('--direction-scale', '-direction_scale', type=float, help='scale direction', default=0, show_default=True)
def generate_images(
        ctx: click.Context,
        network_pkl: str,
        cfg: str,
    ...
    run_dir = gen_utils.make_run_dir(outdir, description, lump_outdir=True)
    ...
    if projected_w is not None:
        ...
        if direction_vec is not None: 
            direction_vec = pickle.load (open(direction_vec,'rb'))
            ws = gen_utils.add_direction_to_w(ws,direction_vec,direction_scale)
        #
        ...
        if ext == '.npy':
            ...
            PIL.Image.fromarray(img, 'RGB').save(f'{run_dir}/from_w_'+re.sub('npy','jpg',projected_w.split('/')[-1])) # use name
        else:
            for idx, w in enumerate(ws):
                img = gen_utils.w_to_img(G, w, noise_mode)[0]
                PIL.Image.fromarray(img, 'RGB').save(f'{run_dir}/from_w_{idx:0{n_digits}d}.jpg')
        return
